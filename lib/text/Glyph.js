...
/**
 * A glyph is a single character of text, rendered as a path of triangles.
 * Each glyph stores a list of "mesh data"
 */
var Glyph = new Class({
    ...
    setup: function(meshes) {
        ...
        ////// TODO: rework this to only rely on triangles... maybe have another class like 
        ///ContourGlyph for polys (mainly useful for stroke)
        ///Or, just make the text triangulated but store the contour as well.
        ///All of this should probably be stored in model-space only. 
        console.log("SETUP");
    ...
    update: function(floor, light) {
        ...
        for (var i=0; i<positions.length; i++) {
            ...
            if (i%3==0) {
                ...
                force.x += Math.sin(force.x * deform * 1);
                force.y += Math.sin(Math.cos(force.x * deform * 1));
                force.z += Math.cos(force.x * deform * 1);

                var scaleRot = 2.1;
                deformMatrix.scale( tmp2.set(scaleRot, scaleRot, scaleRot) );
                deformMatrix.translate( origin );
                deformMatrix.rotateY(deform * force.x * rsc);
                deformMatrix.rotateX(deform * force.y * rsc);
                deformMatrix.rotateZ(deform * force.z * rsc);
                deformMatrix.translate( tmp2.set(origin).negate() );
                deformMatrix.scale( tmp2.set(1/scaleRot, 1/scaleRot, 1/scaleRot) );

                var dsc = 1 - (deform*0.5);
                deformMatrix.scale( tmp2.set(dsc, dsc, dsc) )


                // scale by "explosion" size
                force.scale(deform * 50);
