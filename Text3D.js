...
function drawTriangles(context, camera, glyph, fill, shadow, points) {
    context.beginPath();
...
var Text3D = new Class({
	...
	initialize: function Text3D(text, face, size, steps, simplify) {
        ...
        this.simplify = typeof simplify === "number" ? simplify : 1;
    ...
    fill: function(context, camera) {
        ...
        for (var i=0; i<glyphs.length; i++) {
            var glyph = glyphs[i];
            drawTriangles(context, camera, glyph, true, false, false);
